...
//
// The browser global names ESLint needs for the no-undef rule. Previously this
// came from "env: browser" in .eslintrc.yml; ESLint 9 flat config expects the
// list to be supplied explicitly, normally by importing the "globals" package.
// This repo has no package.json, and pre-commit resolves eslint.config.js
// imports from the repo root rather than from the hook's own environment, so
// the list is vendored here instead and imported by relative path.
//
// Source: globals@15.15.0, the globals.browser export. To refresh against a
// newer release:
//
//   npx --yes -p globals@latest node -p '
//     Object.entries(require("globals").browser)
//       .map(([k, v]) => `    ${k}: "${v ? "writable" : "readonly"}",`)
//       .join("\n")'
//
// and paste the result between the braces below.
